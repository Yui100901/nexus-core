...
// 将指定的许可证与节点进行绑定
func (s *NodeService) AddBinding(ctx context.Context, nodeId uint, binding *entity.NodeBinding) error {
	node, err := s.nr.GetByID(ctx, nodeId)
	if err != nil {
		return err
	}
	node.Bind(binding)
	return s.nr.AddBinding(ctx, nodeId, binding)
}

// UpdateBindingStatus 更新绑定状态
// 修改节点与许可证之间的绑定状态
func (s *NodeService) UpdateBindingStatus(ctx context.Context, id uint, status int) error {
	return s.nr.UpdateBindingStatus(ctx, id, status)
}
...
func (s *NodeService) ForceUnbindByNodeAndLicense(ctx context.Context, nodeID, licenseID uint) error {
	// 查找节点和许可证之间的绑定关系
	binding, err := s.nr.GetBindingByNodeAndLicense(ctx, nodeID, licenseID)
	if err != nil {
		return err
	}
